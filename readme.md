**Проблема**: атомарно обновить БД и отправить сообщение по сети (например в кафку)<br>
Когда внешний вызов (RPC/HTTP) происходит внутри блока @Transactional, откат базы данных никак не влияет на то, что запрос уже улетел в сеть.
